                                                    ...
                                                    std::size_t srcOffset)
{
   ...
   lseek(srcFd, srcOffset, SEEK_SET);
   std::size_t dataCopied = 0;
   std::size_t leftToCopy = srcSize;
   while (dataCopied < srcSize) {
      auto ret = copy_file_range(srcFd, NULL, destFd, NULL, leftToCopy, 0);
      if (ret < 0) {
         throw std::runtime_error("failed to copy from src to dest (" + std::string(strerror(errno)) + ")");
         break;
      }
