...
std::vector<std::string> ReadSpeed::RSTTree::GetMatchingBranchNames(const std::string &fileName,
...
std::vector<std::vector<std::string>> GetPerFileBranchNames(const Data &d)
{
   ...
   for (const auto &fName : d.fFileNames) {
      std::vector<std::string> branchNames;
      if (d.fUseRegex) {
         branchNames = GetMatchingBranchNames(fName, d.fTreeOrNTupleNames[treeOrNTupleIdx], regexes);
      } else
         branchNames = d.fBranchOrFieldNames;
...
ByteData ReadSpeed::RSTTree::ReadTree(TFile *f, const std::string &treeName,
...
Result ReadSpeed::RSTTree::EvalThroughputST(const Data &d)
{
   auto treeIdx = 0;
   auto fileIdx = 0;
   ...
   for (const auto &fileName : d.fFileNames) {
      ...
      const auto byteData = ReadTree(f.get(), d.fTreeOrNTupleNames[treeIdx], fileBranchNames[fileIdx]);
...
// d.fFileNames.
std::vector<std::vector<EntryRange>> ReadSpeed::RSTTree::GetClusters(const Data &d)
{
   ...
   for (auto fileIdx = 0u; fileIdx < nFiles; ++fileIdx) {
      ...
      const auto &treeName = d.fTreeOrNTupleNames.size() > 1 ? d.fTreeOrNTupleNames[fileIdx] : d.fTreeOrNTupleNames[0];
...
ReadSpeed::RSTTree::MergeClusters(std::vector<std::vector<EntryRange>> &&clusters, unsigned int maxTasksPerFile)
{
...
Result ReadSpeed::RSTTree::EvalThroughputMT(const Data &d, unsigned nThreads)
{
#ifdef R__USE_IMT
...
Result ReadSpeed::RSTTree::EvalThroughput(const Data &d, unsigned nThreads)
{
